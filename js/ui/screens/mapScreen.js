// Begehbare Stadtkarte im Pokemon-Stil (Nutzer-Wunsch): Mayo laeuft per
// Pfeiltasten ueber eine illustrierte Stadtkarte, Landmarks sind zusaetzlich
// direkt anklickbar (schnellerer Zugriff, kein exaktes "Draufsteppen"
// noetig). Betreten eines Landmarks fuehrt in die bestehende
// Szenen-Hotspot-Ansicht (sceneScreen.js) fuer diese Location.
...
function render(level, karte, state) {
  ...
  containerEl.style.backgroundImage = `url(${karte.background})`;
  ...
  for (const landmark of karte.landmarks) {
    ...
    btn.style.left = `${(landmark.position.x / karte.breiteInTiles) * 100}%`;
    btn.style.top = `${(landmark.position.y / karte.hoeheInTiles) * 100}%`;
...
function positioniereMayoToken(el, karte, state) {
  el.style.left = `${(state.kartenPosition.x / karte.breiteInTiles) * 100}%`;
  el.style.top = `${(state.kartenPosition.y / karte.hoeheInTiles) * 100}%`;
}
